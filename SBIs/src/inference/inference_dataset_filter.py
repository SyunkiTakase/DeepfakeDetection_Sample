...
def low_pass_filter(image_tensor, cutoff_frequency):
    fft_image = fft.fft2(image_tensor, dim=(-2, -1))
    fft_image_shifted = fft.fftshift(fft_image, dim=(-2, -1))
    ...
    low_pass_filter = (distance <= cutoff_frequency).float().to('cuda')
    fft_image_low_pass = fft_image_shifted * low_pass_filter.unsqueeze(0).unsqueeze(0)
    
    image_low_pass = fft.ifft2(fft.ifftshift(fft_image_low_pass, dim=(-2, -1)), dim=(-2, -1))
    image_low_pass = image_low_pass.real
    
    return image_low_pass

def high_pass_filter(image_tensor, cutoff_frequency):
    fft_image = fft.fft2(image_tensor, dim=(-2, -1))
    fft_image_shifted = fft.fftshift(fft_image, dim=(-2, -1))
    ...
    high_pass_filter = (distance > cutoff_frequency).float().to('cuda')
    fft_image_high_pass = fft_image_shifted * high_pass_filter.unsqueeze(0).unsqueeze(0)

    image_high_pass = fft.ifft2(fft.ifftshift(fft_image_high_pass, dim=(-2, -1)), dim=(-2, -1))
    image_high_pass = image_high_pass.real
...
def main(args):
    ...
    # save_path = '/home/tr22008/deepfake_detection/SelfBlendedImages-master/output/sbi_base3_12_25_13_37_53/AUC_high/'
    save_path = '/home/tr22008/deepfake_detection/SelfBlendedImages-master/output/sbi_base3_12_25_13_37_53/AUC_low/'
    ...
    for filename in tqdm(video_list):
        try:
            ...
            with torch.no_grad():
                ...
                if args.filter_type == 'high':
                    filtered_img = high_pass_filter(img, cutoff_frequency)
                elif args.filter_type == 'low':
                    filtered_img = low_pass_filter(img, cutoff_frequency)

                pred=model(filtered_img).softmax(1)[:,1]
                # print(img.shape)
...
if __name__=='__main__':
    ...
    parser.add_argument('-type',dest='filter_type',type=str)
